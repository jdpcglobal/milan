...
const ProfileScreens = () => {
  ...
  //  console.log('>>>>>>>>>>>123',heightTo);
  // console.log('>>>>>>>>>>>123333', jobTitle);
  ...
  const profileData = async () => {
    ...
    try {
      ...
      setHeight(`${data.data.height ? data.data.height : ''}`);
      ...
      setAddress(data?.data?.living && data.data.living !== "null" ? data.data.living : "");
      // setJobTitle(`${data.data.jobTitle}`);
      setJobTitle(
        data?.data?.jobTitle && data.data.jobTitle !== "null"
          ? data.data.jobTitle
          : ""
      );
      ...
      setHeightTo(`${data.data.heightTo}`);
      setHeightFrom(`${data.data.heightFrom}`);
      setSelectedItems(data.data.interests);
      setSelectedBio(data.data.bio);
      // console.log('>>>>>>>>>>>22222', data.data.bio);
  ...
  const InterestApi = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('getInterest Error')
      }
      const data = await response.json()
      ...
      console.error('getInterest Error Last')
    }
  };
          ...
          <View style={{ backgroundColor: '#fafafa', paddingVertical: 15, borderRadius: 10 }}>
            ...
            <View>
              <View style={styles.OptionsView}>
                <View >
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 5 }}>
                    <Ionicons style={{ marginTop: 15 }} name={'flower-outline'} size={25} color={'#4A4744'} /><Text style={styles.slideHeader}> Select your interests</Text>
                  </View>
                ...
                <View >
                  ...
                  <View style={{ width: 'auto', flexDirection: 'row', flexWrap: 'wrap', marginLeft: 20 }}>
                    ...
                    <TouchableOpacity onPress={() => handleTextClick('2')}>
                      <Text style={[styles.slideText, focusedText === '2' && styles.focusedText]}> Long-term but short-term ok</Text>
                    </TouchableOpacity>
            ...
            <View style={{
              ...
              <View >
                <Text style={[styles.slideHeader, { marginLeft: 10, marginBottom: 20 }]}>Select Height preference </Text>
                <View style={{ marginTop: 2 }}>
            ...
            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
              {isLoading ? (
                ...
                <View style={{ width: SCREEN_WIDTH / 1.3, margin: 20, padding: 5, }}>
                  <AnimatedButton
                    title="Submit"
...
const styles = StyleSheet.create({
  ...
  textInputFocused: {
    backgroundColor: '#f0f8ff', // Light blue background when focused
